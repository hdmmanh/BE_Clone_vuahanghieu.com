...
<div class="wrapper">
    ...
    <div class="dashboard container-fluid ps-0 pe-0">
        <div class="site-bar" style="height: 657.4px">
            <ul>
        ...
        <div class="content-admin">
            <h3 style="color:aliceblue " class="ms-3 mt-3 mb-3"> Chi tiết sản phẩm</h3>
            <div class="table-content">
                <table class="table table-dark table-striped mb-0 table-chi-tiet-san-pham">
                    <thead>
                    <tr>
                        <th scope="col" style="width: 30px;padding: 0">STT</th>
                        <th scope="col" style="width: 30px;padding: 0">ID</th>
                        <th scope="col" style="width: 270px">Tên sản phẩm</th>
                        <th scope="col" style="width: 55px;padding: 0">Màu</th>
                        <th scope="col" style="width: 46px;padding: 0">Size</th>
                        <th scope="col" style="width: 80px;padding: 0">Giá bán</th>
                        <th scope="col" style="width: 60px;padding: 0">Số lượng tồn</th>
                        <th scope="col" style="width: 55px;padding: 0">Trạng thái</th>
                        <th scope="col" style="width: 80px;padding: 0">Hành động</th>
                    </tr>
                    </thead>
                    <tbody>
                    <c:forEach items="${listCTSP}" var="ctsp" varStatus="i">
                        <tr>
                            ...
                            <td style="text-align: center">${ctsp.id}</td>
                            <td style="text-align: center">${ctsp.sanPham.ten_san_pham}</td>
                            <td style="text-align: center">${ctsp.mauSac.ten_mau}</td>
                            <td style="text-align: center">${ctsp.size.ten_size}</td>
                            <td style="text-align: center">
                                <fmt:formatNumber value="${ctsp.gia_ban}" pattern="#,##0"/>
                                <spacer>₫</spacer>

                            </td>
                            <td style="text-align: center">${ctsp.so_luong_ton}</td>
                            <td style="text-align: center">${ctsp.trang_thai}</td>
                            <td class="button-action">
                                <a href="" class="btn btn-info me-3">Chi tiết</a>
                        ...
                        <div>
                            <c:choose>
                                <c:when test="${currentPage > 1}">
                                    <a href="/page1/${currentPage - 1}" class="button-prev">Previous</a>
                                </c:when>
                                <c:otherwise>
                                    ...
                                    <c:when test="${i <= 3 or i > (totalPages - 3)}">
                                        <c:choose>
                                            <c:when test="${currentPage != i}">
                                                <a href="/page1/${i}" class="button-page">${i}</a>
                                            </c:when>
                                            <c:otherwise>
                        ...
                        <div>
                            <c:choose>
                                <c:when test="${currentPage < totalPages}">
                                    <a href="/page1/${currentPage + 1}" class="button-next">Next</a>
                                </c:when>
                                <c:otherwise>
